  ...
  <div class="min-h-[calc(100vh-64px)] bg-[#FAFAFA] text-zinc-900 font-sans flex flex-col justify-center items-center px-4 py-6 sm:py-10">
    <div class="w-full max-w-[440px] mx-auto my-auto">
      ...
      <div class="text-center mb-6">
        <h1 class="text-2xl sm:text-[26px] font-bold tracking-tight text-zinc-900 mb-1.5">{{ $t('auth.register.title') }}</h1>
        <p class="text-[13.5px] text-zinc-500">{{ $t('auth.register.subtitle') }}</p>
      </div>
      ...
      <div class="rounded-2xl border border-zinc-200 bg-white shadow-card p-6 sm:p-7">
        ...
        <form @submit.prevent="handleSubmit" novalidate class="space-y-4">
          <!-- Username -->
          <div>
            <label for="username" class="block text-[12.5px] font-medium text-zinc-700 mb-1.5">{{ $t('auth.register.nameLabel') }}</label>
            <input
              id="username"
            ...
            <Transition enter-from-class="opacity-0 -translate-y-0.5" enter-active-class="transition duration-150">
              <p v-if="fieldErrors.username" class="mt-1.5 text-[11.5px] text-red-600">
                {{ fieldErrors.username }}
              </p>
            </Transition>
          </div>
          ...
          <!-- Email -->
          <div>
            <label for="email" class="block text-[12.5px] font-medium text-zinc-700 mb-1.5">{{ $t('auth.register.emailLabel') }}</label>
            <input
              id="email"
              v-model="form.email"
              type="email"
          ...
          <!-- Password -->
          <div>
            <label for="password" class="block text-[12.5px] font-medium text-zinc-700 mb-1.5">{{ $t('auth.register.passwordLabel') }}</label>
            <div class="relative">
              <input
                id="password"
          ...
          <!-- Confirm Password -->
          <div>
            <label for="confirmPassword" class="block text-[12.5px] font-medium text-zinc-700 mb-1.5">{{ $t('auth.register.confirmPasswordLabel') }}</label>
            <div class="relative">
              <input
                id="confirmPassword"
            ...
            <Transition enter-from-class="opacity-0 -translate-y-0.5" enter-active-class="transition duration-150">
              <p v-if="fieldErrors.confirmPassword" class="mt-1.5 text-[11.5px] text-red-600">
                {{ fieldErrors.confirmPassword }}
              </p>
            </Transition>
          </div>
      ...
      <p class="text-center text-[13px] text-zinc-500 mt-5">
        ...
        <NuxtLink to="/auth/login" class="font-semibold text-zinc-900 hover:underline transition-all ml-1">
...
useHead({
  title: computed(() => locale.value === 'th' ? 'สมัครใช้งาน - PicHost.io' : 'Sign Up - PicHost.io'),
  htmlAttrs: {
...
async function handleSubmit() {
  ...
  if (!form.username) {
    fieldErrors.username = locale.value === 'th' ? 'กรุณากรอกชื่อของคุณ' : 'Name is required'
  }
  if (!form.email) {
  ...
  if (!form.password) {
    ...
    fieldErrors.password = locale.value === 'th' ? 'รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร' : 'Password must be at least 8 characters'
  }

  if (!form.confirmPassword) {
    fieldErrors.confirmPassword = locale.value === 'th' ? 'กรุณายืนยันรหัสผ่าน' : 'Confirm password is required'
  } else if (form.password !== form.confirmPassword) {
    fieldErrors.confirmPassword = locale.value === 'th' ? 'รหัสผ่านไม่ตรงกัน' : 'Passwords do not match'
  }
  ...
  try {
    ...
    await fetchMe()
    toastSuccess(locale.value === 'th' ? 'สมัครสมาชิกสำเร็จ' : 'Account created successfully')
