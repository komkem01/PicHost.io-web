    ...
    <main class="max-w-4xl mx-auto px-6 py-12 sm:py-16">
      ...
      <div class="mb-10 text-center sm:text-left">
        ...
        <h1 class="text-3xl sm:text-4xl font-extrabold tracking-tight text-zinc-900 mb-3">
          {{ doc?.title || $t('legal.privacyTitle') }}
        </h1>
        <p class="text-zinc-500 text-sm sm:text-base">
          {{ doc?.updated_at ? `Last Updated: ${formatDate(doc.updated_at)}` : 'Last Updated: August 8, 2026' }} &bull; Compliant with Thailand PDPA.
        </p>
      </div>
...
async function fetchDoc() {
  loading.value = true
  try {
    const res = await apiFetch<any>('/public/legal/privacy')
    if (res.data) {
      doc.value = res.data
    }
  } catch (e) {
    console.error('Failed to load privacy document:', e)
  } finally {
    loading.value = false
  }
}
...
useSeoMeta({
  title: 'Privacy Policy — PicHost.io',
  description: 'Learn how PicHost.io protects your privacy and personal data.',
})
