              ...
              <div class="relative aspect-square bg-white/[0.04] flex items-center justify-center overflow-hidden">
                ...
                <div class="absolute inset-0 bg-black/55 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  ...
                  <button
                    ...
                    title="Delete"
                    class="w-8 h-8 rounded-lg bg-red-500/20 hover:bg-red-500/40 border border-red-500/25 flex items-center justify-center transition-colors"
                  >
...
async function deleteFile(file: StorageFile) {
  ...
  try {
    await $fetch(`${config.public.apiBase}/storage/files/${file.storage_id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${getToken()}` },
    })
